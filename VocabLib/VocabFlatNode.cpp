...
PushAndScoreFeature(
    unsigned char *descriptor, unsigned int index,
  ...
  if (USE_SOFT_ASSIGNMENT) {
    ...
    for (int i = 0; i < NUM_NNS; i++) {
      ...
      double w_weight = w_weights[i];
      r = m_children[nn_idx[i]]->PushAndScoreFeature(
          descriptor, index,
          bf, dim, add);
    }
  } else {
    r = m_children[nn_idx[0]]->PushAndScoreFeature(descriptor, index, bf, dim, add);
  }
